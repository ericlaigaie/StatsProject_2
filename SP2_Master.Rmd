...
x<-t(cluster.train.x)
colnames(x)<-cluster.train.y

pheatmap(x,annotation_col=data.frame(Pay=cluster.train.y),annotation_colors=list(Pay=c("<=50K"="white",">50K"="green")),scale="row",legend=T,color=colorRampPalette(c("blue","white", "red"), space = "rgb")(30000))
...
#Mosaic Plot
#Didn't observe any collinearity among the workclass and education variables.

t(aggregate(workclass~education,data=df_trainclean,summary))
plot(df_trainclean$workclass~df_trainclean$education,col=c("orange","blue", "green", "red", "yellow"))

count <- table(df_trainclean$workclass, df_trainclean$education)
plot(count, col=c("orange","blue", "green", "red", "yellow", "purple"))
...
plot(df_trainclean$marital_status~df_trainclean$education,col=c("orange","blue"))


count <- t(table(df_trainclean$marital_status, df_trainclean$education))
plot(count, col=c("orange","blue", "green", "red"))
...
plot(df_trainclean$race~df_trainclean$education,col=c("orange","blue"))

count <- t(table(df_trainclean$race, df_trainclean$education))
plot(count, col=c("orange","blue", "green", "red", "yellow"))
...
#No evidence of collinearity among race and marital status
...
full.log<-glm(pay_response~.-id,family="binomial",data=df_trainclean)
...
lda.fit = lda(pay_response~., data = df_trainclean[,-1])

lda.fit

##Predict Training Results
predmodel.train.lda <-  predict(lda.fit, data=df_trainclean)
predmodel.train.lda

# Create table to get prediction accuracy
table(Predicted <- predmodel.train.lda$class, pay_response = df_trainclean$pay_response)
conf_matrix_train <-  table(Predicted <- predmodel.train.lda$class, pay_response = df_trainclean$pay_response)
...
#Test
predmodel.test.lda = predict(lda.fit, newdata=df_testclean)
table(Predicted=predmodel.test.lda$class, pay_response=df_testclean$pay_response)

conf_matrix_test <- table(Predicted=predmodel.test.lda$class, pay_response=df_testclean$pay_response)
...
#ROC for LDA
preds <- predmodel.test.lda$posterior
preds <- as.data.frame(preds)

pred <- prediction(preds[,2],df_test2$pay_response)
...
plot(roc.step,col="orange", add = TRUE)
legend("bottomright",legend=c("LDA","Lasso","Stepwise"),col=c("black","green", "orange"),lty=1,lwd=1)
abline(a=0, b= 1)
...
# LOG of LDA code since assumptions failed for LDA with variables
# LDA LOGIC

```{r}
df_traincleanlog <- df_trainclean[, c(2, 4, 11, 13, 14)]
df_testcleanlog <- df_testclean[, c(2, 4, 11, 13, 14)]

df_traincleanlog$logage <-  log(df_traincleanlog$age)
df_traincleanlog$logfnlwgt <-  log(df_traincleanlog$fnlwgt)
...
df_testcleanlog$logfnlwgt <-  log(df_testcleanlog$fnlwgt)
...
ggarrange(lda_eq_var1("logage"),lda_eq_var1("logfnlwgt"),lda_eq_var1("loghours_per_week"),lda_eq_var1("logcapital_net"),nrow=2,ncol=2)
...
ggarrange(lda_eq_var_cov1("logage","logfnlwgt"), 
          lda_eq_var_cov1("logage","loghours_per_week"),
          lda_eq_var_cov1("logage","logcapital_net"),
          lda_eq_var_cov1("logfnlwgt","loghours_per_week"),
          lda_eq_var_cov1("logfnlwgt","logcapital_net"),
...
variable_2 <- c("logage", "logfnlwgt", "loghours_per_week", "logcapital_net") 
par(mfrow = c(2, 2)) 
for(i in variable_2) { 
    qqnorm(pay.yes[[i]]); qqline(pay.yes[[i]], col = 2) 
}
...
lda.fit = lda(pay_response~., data = df_traincleanlogx[,c(4,6,7,8,9)])
...
```{r}
qda.fit = qda(pay_response~., data = df_traincleanlogx[,c(4,6,7,8,9)])
...
# Interaction Model
```{r}
# fit.int <- glm(pay_response ~ age + hours_per_week + capital_net + sex + sex:age + marital_status:occupation + workclass:sex + capital_net:age, family='binomial', data=df_trainclean) # 82.25% accuracy


# fit.int <- glm(pay_response ~ age + hours_per_week + capital_net + sex + sex:age + marital_status:occupation + workclass:sex + capital_net:age + capital_net:workclass + native_country:sex, family='binomial', data=df_trainclean) # 82.97% accuracy

# fit.int <- glm(pay_response ~ age + hours_per_week + capital_net + sex + sex:age + marital_status:occupation + workclass:sex + capital_net:age + capital_net:workclass + native_country:sex + education, family='binomial', data=df_trainclean) # 83.96% accuracy

fit.int <- glm(pay_response ~ age + hours_per_week + capital_net + sex + sex:age + marital_status:occupation + workclass:sex + capital_net:age + capital_net:workclass + native_country:sex + education + education:age + hours_per_week:capital_net, family='binomial', data=df_traincleanlog)

# I'm not really impressed with the accuracy here....not sure if I need to be using log data
...
ggplot(int.cutoff_acc_loop, aes(x=cutoffs, y=accuracies, label=accuracies)) + 
  geom_line(size=2, color='indianred2') +
  labs(x='Classiciation Cutoff', y='Accuracy', title='Accuracy by Cutoff Value - Interactin Based') +
  geom_point(aes(y=int.maxacc, x=int.maxcut), size=5, color='black') +
  geom_text(
    aes(label=
          ifelse(accuracies == int.maxacc,
